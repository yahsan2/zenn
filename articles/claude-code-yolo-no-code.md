---
title: "あなたはYOLO派？ - Claude Codeで実践する「コードを書かない開発」"
emoji: "🔄"
type: "idea"
...
ということではなく、品質を保って、コードは8割把握を目指します。

私がやるのは、次のことです。

- 入力指示をを与えてプランを書き、プランに基づき実装させる
...
私はこれを直さず、コードの修正指示を出ないことも多くあります。
...
```
## ズレたら、理由を聞く
これも自分はコマンドとして用意しています。ここでは省略します。
メタコマンド・メタスキルの話は、要望あれば別の記事に書くのでフォローよろしくお願いします。
```
...
## **YOLOデフォルト派** なのは「学習ループ」

YOLOというと、無計画・危険という印象を持たれがちですが、私にとってのYOLOは違います。

> **YOLOとは、雑に出して、観測して、入力を更新しやすくなります**
...
graph TD
    ...
    B -.->|出力のズレを見る| B1((差分))
    C -.->|なぜズレた？| C1((理由))
    D -.->|CLAUDE.md / Rules / Commands| D1((資産))
```

---

## 同じ設計書（plans.md）を、何度も使う理由

設計書（私は、同じplans.md）を何度も書き換え、何度も使います。

- 再現できるかを試す
- 出力のブレを観測する
- 設計の弱点を炙り出す

設計書（そして、検証が済んだplans.md）は、スキルやコマンドに変換させます。

設計書（plans.md）は使い捨てではなく、**再利用可能な資産**です。

---

## 設計書（plans.md）は、コマンドに変換される

設計書から、command / skill / subagent を切り出させるのは難しくありません。

設計書（たとえば、APIエンドポイントを作るplans.md）があるとします。
