---
title: "個人用とクライアント（会社）用の Claude Code を併用する方法 - Dev Container + ボリューム分離で安全に"
emoji: "🔐"
type: "tech"
...
クライアントごとにアカウントを分けると、毎回 `claude /logout` → `claude /login` の切り替えが必要になります。Dev Container + ボリューム分離なら、**コンテナを切り替えるだけで自動的にアカウントも切り替わる**ため、この手間がなくなります。
...
2025/12/05 時点で、ローカルの `~/.claude/commands/` に定義したカスタムスラッシュコマンドは、**CLI では使えますが、VS Code 拡張機能では認識されません**。
...
- **ログイン切り替え不要**: コンテナを切り替えるだけでアカウントも自動切り替え
