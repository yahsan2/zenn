...
### シナリオ 3: 「クライアントへの請求、どう証明する？」

Max プランで Claude Code を使っているが、複数クライアントの作業を同じアカウントでやっていると、どのクライアントにどれだけ使ったか分からなくなる。

**問題点**:
- API 使用量をクライアントごとに分けられない
- 「御社の作業でこれだけ使いました」と証明できない
- 経費精算や請求書作成時に困る
