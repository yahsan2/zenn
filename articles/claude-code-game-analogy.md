...
※ 実際にはスキルやサブエージェントは `.claude/skills/` や `.claude/agents/` に配置すれば**自動発見**されます。CLAUDE.mdに書かなくても使えますが、重要なものや頻度が高いものは使用する確率を上げるために書いておくはアリです。「どんな時に使うか」はスキル定義ファイルの `description` に書きましょう。
