...
:::message alert
**2026年1月8日 最新アップデート情報**

Skills機能に重要な変更がありました：
- **Skillsがスラッシュコマンドメニューから直接呼び出せるように**なりました
- デフォルトでは、全てのSkillがスラッシュコマンドメニューに表示されます
- `user-invocable: false` をフロントマターに設定することで、メニューからの表示/非表示を制御できます
- これにより、**Skillsは引き続きモデル呼び出し（Claudeが自動判断）として機能しつつ、ユーザーが明示的に呼び出すこともできる**ようになりました
...
# 利用可能なサブエージェント
- mage: 魔法で攻撃する専門家
- guide: モンスターの居場所を案内する

# 利用可能なスキル
- thunder-magic: サンダー魔法
- blizzard-magic: ブリザド魔法
